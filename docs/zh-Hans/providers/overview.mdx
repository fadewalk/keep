...
title: "概述"
sidebarTitle: "概述"
description: "Provider 是 Keep 的一个组件，使 Keep 能够与第三方产品交互。它以可扩展的 Python 代码实现，易于增强和定制。"
---

Provider 是 Keep 的核心组件，允许 Keep 查询数据、发送通知、获取告警或管理第三方工具。

这些第三方工具包括但不限于 Datadog、Cloudwatch 和 Sentry，用于数据查询和/或告警管理，以及 Slack、Resend、Twilio 和 PagerDuty 用于通知/事件。

通过利用 Keep Provider，用户能够将 Keep 与他们使用和信任的工具深度集成，为他们提供灵活而强大的方式来轻松地从单一界面管理这些工具。
