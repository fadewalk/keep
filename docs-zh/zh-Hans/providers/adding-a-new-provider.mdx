...
- [身份验证 configuration](#authentication-configuration)
...
- 该 Provider type is automatically extracted from the class name (for example, `ServiceNowProvider` → `servicenow`).
...
```python
"""
YourService Provider is a class that 允许 integration with YourService.
"""

import dataclasses
import json
import os
from typing import 可选, List, Dict, Any
...
class YourserviceProviderAuthConfig:
    ...
    api_endpoint: str = dataclasses.field(
        metadata={
            ...
            "validation": "https_url",  # 可选: validates HTTPS URLs
        }
    )
...
class YourserviceProvider(BaseProvider):
    ...
    # 必填: Display name shown in UI
    PROVIDER_DISPLAY_NAME = "YourService"
    
    # 必填: Categories for provider classification
    PROVIDER_CATEGORY = ["Monitoring"]
    
    # 可选: Tags for searchability
    PROVIDER_TAGS = ["alert", "data"]
    
    # 可选: Define required scopes/permissions
    PROVIDER_SCOPES = [
        ProviderScope(
            name="read:alerts",
        ...
        ProviderScope(
            ...
            mandatory=False,
            mandatory_for_webhook=True,  # 必填 only for webhook setup
        ),
    ]
    
    # 可选: OAuth2 URL (MUST be set as class attribute, not in __init__)
    ...
    def __init__(
        ...
        # 注意： Logger is automatically available as self.logger
        
        # Context manager 提供 access to:
...
该 Provider type is automatically extracted from your class name:
...
- `"topology"` - Provider 提供 topology data
...
class ProviderScope:
    ...
    Args:
        ...
        description (可选[str]): The description of the scope.
        ...
        documentation_url (可选[str]): The documentation url of the scope.
        alias (可选[str]): Another alias of the scope.
    """

    name: str
    description: 可选[str] = None
    mandatory: bool = False
    mandatory_for_webhook: bool = False
    documentation_url: 可选[str] = None
    alias: 可选[str] = None
```
...
class ProviderConfig:
    ...
    Args:
        description (可选[str]): The description of the provider.
        authentication (dict): The configuration for the provider.
    """

    authentication: 可选[dict]
    name: 可选[str] = None
    description: 可选[str] = None
...
class BaseProvider(metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        webhook_template: 可选[str] = None,
        webhook_description: 可选[str] = None,
        webhook_markdown: 可选[str] = None,
        provider_description: 可选[str] = None,
    ):
        """
        Initialize a provider.

        Args:
            provider_id (str): 该 Provider id.
    ...
    def notify(self, **kwargs):
        ...
        Args:
            **kwargs (dict): 该 Provider context (with statement)
        """
    ...
    def _enrich(self, enrichments, results, audit_启用d=True):
        """
        ...
        Args:
            ...
            audit_启用d: Whether to audit the enrichment operation (default: True)
        """
    ...
    def _format_alert(
        ...
        Args:
            ...
            provider_instance: 可选 provider instance for context
    ...
    def get_alerts_configuration(self, alert_id: 可选[str] = None):
        """
        Get configuration of alerts from the provider.

        Args:
            alert_id (可选[str], optional): If given, gets a specific alert by id. Defaults to None.
        """
    ...
    def deploy_alert(self, alert: dict, alert_id: 可选[str] = None):
        """
        Deploy an alert to the provider.

        Args:
            alert (dict): The alert to deploy.
            alert_id (可选[str], optional): If given, deploys a specific alert by id. Defaults to None.
        """
    ...
    def get_alert_schema() -> dict:
        ...
        Get the alert schema description for the provider.
            e.g. 如何 define an alert for the provider that can be pushed via the API.
...
2. **BaseTopologyProvider** (`keep/providers/base/base_provider.py`)
   ...
   - 示例: Datadog Provider (`keep/providers/datadog_provider/datadog_provider.py`)
...
3. **BaseIncidentProvider** (`keep/providers/base/base_provider.py`)
   ...
   - 示例: PagerDuty Provider (`keep/providers/pagerduty_provider/pagerduty_provider.py`)
...
Keep 提供 specialized base classes for specific provider types:
...
class MyIncidentProvider(BaseIncidentProvider):
    ...
    def setup_incident_webhook(
        ...
        """
        设置 webhook for incident updates.
...
注意： The `get_incidents()` method is automatically provided by the base class and wraps `_get_incidents()`. The `format_incident()` class method handles provider loading and calls `_format_incident()`.
...
class MyProviderAuthConfig:
    """配置 for MyProvider authentication."""
...
Keep 提供 built-in field validation through custom Pydantic field types:

| Validation Type | Field Type | Description | 示例 |
|----------------|------------|-------------|---------|
...
你可以 test your provider by running it directly:
```bash
cd keep
python -m keep.providers.yourservice_provider.yourservice_provider
```

The `if __name__ == "__main__":` block 允许 you to test provider initialization and basic capabilities.
...
class YourserviceProvider(BaseProvider, ProviderHealthMixin):
    ...
    # The mixin 提供 automatic health checking for:
...
This 允许 workflows to access the actual timestamps used in queries, not just the relative time strings.
...
- Understand [Linked vs 连接ed Providers](/providers/linked-providers)
