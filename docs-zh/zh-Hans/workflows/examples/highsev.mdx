...
工作流链接：[workflow](https://github.com/keephq/keep/blob/main/examples/workflows/create_jira_ticket_upon_alerts.yml)。

</Info>

此工作流根据关联的服务处理来自 Sentry 的严重告警。




说明：
- 触发器：由来自 Sentry 的严重告警激活。
- 动作：
- - 对于与 `payments` 服务相关的告警，向 payments 团队发送 Slack 消息。
- - 对于与 `ftp` 服务相关的告警，如果尚不存在工单 ID，则创建 Jira 工单。
