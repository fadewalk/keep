...
工作流链接：[workflow](https://github.com/keephq/keep/blob/main/examples/workflows/update_service_now_tickets_status.yml)。

</Info>

此示例演示如何定期更新与告警关联的 ServiceNow 工单的状态。

说明：
- 触发器：工作流可以手动触发，模拟计划的执行。
- 步骤 1：使用 Keep 提供者获取所有 `ticket_type` 为 `servicenow` 的告警。
- 动作：遍历获取的告警，并使用最新状态更新其关联的 ServiceNow 工单。
